/* ミニマ判定(純粋関数)。AIは使わず、値としきい値だけで決める。
   ...
   ミニマ(設定画面で入力、端末内だけに保存):
     ...
     sctCautionFt(この高さ以下のSCTは注意、既定1000),
...
export function judgeConditions(c, minima, { runways = [], temp = null, dew = null } = {}) {
  const m = {
    ...
    nogoWx: minima.nogoWx || [], spreadC: num(minima.spreadC) ?? 3, sctCautionFt: num(minima.sctCautionFt) ?? 1000,
  };
  const items = [];
  ...
  /* 低いSCT：シーリングにはならないが、雲底の下を飛ぶヘリには低い雲がかかるので注意 */
  const lowSct = (c.clouds || []).filter(l => l.cover === 'SCT' && l.base != null && l.base <= m.sctCautionFt);
  if (lowSct.length) items.push({ key: 'lowsct', label: '低い雲(SCT)', value: `SCT ${Math.min(...lowSct.map(l => l.base))}ft`, level: 'caution', why: `${m.sctCautionFt}ft以下のSCT` });
...
/* VMC / SVFR / IMC。
   - VMC：航空法施行規則第5条第4号(管制圏・情報圏内の空港等で離着陸する航空機)の地上の基準。
     雲高(BKN/OVC/VV の一番低い層)300m≒1,000ft 以上、かつ地上視程 5km 以上。
     ⚠ 告示で指定された空港は 8km・450m。空域・高度で基準が違い、雲からの距離など地上観測で決まらない条件もある
   - SVFR：IMC のうち地上視程 1,500m 以上。管制業務処理規程(Ⅲ(Ⅱ)3(1)a)で、管制圏・情報圏のある飛行場の
     地上視程が1,500m以上なら管制機関は交通状況を考慮して特別有視界飛行方式を許可できる。雲高の条件は無い。
     飛行中は則198条の4(雲から離れる・飛行視程1,500m以上・地表視認・管制機関と常時連絡)。許可が要る
   - IMC：地上視程 1,500m 未満
   設定 vmcCeilFt・vmcVisM・svfrVisM で変えられる。
   欠測：分かっている要素で決まるなら決め、決まらなければ null(不明) */
export const FR_RANK = { VMC: 0, SVFR: 1, IMC: 2 };
export function flightRule(c, minima = {}) {
  const ceilMin = num(minima.vmcCeilFt) ?? 1000, visMin = num(minima.vmcVisM) ?? 5000, svfrMin = num(minima.svfrVisM) ?? 1500;
  ...
  if (why.length) {
    if (v == null) return { rule: 'IMC', why: `${why.join('・')}（視程が欠測のためSVFRかどうか不明）` };
    if (v >= svfrMin) return { rule: 'SVFR', why: `${why.join('・')}・地上視程${v}m ≥ ${svfrMin}m（許可があれば特別VFR）` };
    return { rule: 'IMC', why: `${why.join('・')}・地上視程${v}m < ${svfrMin}m（特別VFRも不可）` };
  }
  if (cl.missing || v == null) return { rule: null, why: '雲か視程が欠測' };
...
/* 数値予報からの推定値の判定(画面6の断面図の色帯)。視程は数値予報に無いので項目から外す */
export function judgeNwp(cond, minima, opt = {}) {
  const r = judgeConditions(cond, minima, opt);
  r.items = r.items.filter(i => i.key !== 'vis');
  r.level = worst(r.items.map(i => i.level));
  r.estimated = true;
  return r;
}
...
export function judgeTaf(taf, minima, { runways = [], from, to } = {}) {
  ...
  const slots = tafTimeline(taf, from, to).map(s => {
    ...
    /* 本体は悪い方(IMC > SVFR > VMC)。欠測があって悪い方が決まらなければ不明 */
    const worstFr = fs => fs.reduce((a, f) => (f.rule && (!a || FR_RANK[f.rule] > FR_RANK[a.rule]) ? f : a), null);
    const pw = worstFr(pre);
    const rule = pw && (pw.rule === 'IMC' || !pre.some(f => f.rule == null)) ? pw.rule : null;
    const tw = worstFr(tmp);
    const tempo = tw && FR_RANK[tw.rule] > (rule ? FR_RANK[rule] : -1) ? tw.rule : null;
    const flight = { rule, why: pw?.why || '', tempo, tempoSrc: tempo ? tmp.filter(f => f.rule === tempo).map(f => f.src.split(' ')[0]).join('・') : '' };
