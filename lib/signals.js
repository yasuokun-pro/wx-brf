/* 総合解析：資料の突き合わせ（純粋関数・DOMを使わない）
   ...
   - 資料が食い違うときは結論を出さず「気象担当者に確認すること」にする
...
export function sigMismatch(nwp, taf, win) {
  ...
  return {
    key: 'mismatch', title: '資料の食い違い（気象担当者に確認）',
    level: 'caution',
    text: `TAFの雲底 ${tafBase}ft と、数値予報の推定雲底 ${Math.round(avg(nwpBase))}ft が ${Math.abs(diff)}ft 違う。どちらを採るかは気象担当者に確認する。`,
...
export function script({ sigs = [], judges = {}, window: win = null, airfields = [], route = null }) {
  ...
  if (asks.length) lines.push({ screen: 0, text: `気象担当者に確認すること：${asks.map(s => s.text).join(' ')}` });
  lines.push({ screen: 0, text: `総合判定は${lv[judges.total] || 'データなし'}です。最終判断は公式の気象ブリーフィングと運航規定に従います。` });
  return lines;
}
