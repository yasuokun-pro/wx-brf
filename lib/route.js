...
/* 気象変換点（峠など）の判定。
   峠を越えるには「雲底(MSL) が 峠の標高＋必要な間隔 より上」であることが要る。
   w = {baseFt(推定雲底 AGL), elevM(峠の標高), windKt, precipMmh, freezingFt, rhAtAlt}
   m = {clearanceFt(峠との必要な間隔、既定500), marginFt(余裕幅、既定500), windKt, precipMmh} */
export function judgePass(w, m = {}) {
  const num = v => (v === '' || v == null || !Number.isFinite(+v) ? null : +v);
  const clr = num(m.clearanceFt) ?? 500, mar = num(m.marginFt) ?? 500;
  const wTh = num(m.windKt) ?? 25, pTh = num(m.precipMmh) ?? 1;
  const elevFt = w.elevM == null ? null : Math.round(w.elevM * M_FT);
  const items = [];
  if (w.baseFt == null || elevFt == null) {
    items.push({ key: 'pass', label: '雲底と峠', value: '—', level: 'none', why: w.baseFt == null ? '雲底を推定できず' : '標高が未設定' });
  } else {
    const baseMsl = w.baseFt + elevFt;          /* 推定雲底は地表からの高さなので峠の標高を足す */
    const need = elevFt + clr;
    const gap = baseMsl - need;
    items.push({
      key: 'pass', label: '雲底と峠', value: `雲底 ${baseMsl}ft MSL ／ 峠 ${elevFt}ft＋間隔 ${clr}ft`,
      level: gap < 0 ? 'nogo' : gap < mar ? 'caution' : 'go',
      why: gap < 0 ? `峠が雲の中（${Math.abs(gap)}ft 足りない）` : gap < mar ? `余裕 ${gap}ft（${mar}ft 未満）` : `余裕 ${gap}ft`,
    });
  }
  if (w.windKt != null) items.push({ key: 'wind', label: '風', value: `${Math.round(w.windKt)}kt`, level: w.windKt >= wTh * 1.4 ? 'nogo' : w.windKt >= wTh ? 'caution' : 'go', why: w.windKt >= wTh ? '山越えの風で乱気流に注意' : '' });
  if (w.precipMmh != null) items.push({ key: 'precip', label: '降水', value: `${w.precipMmh}mm/h`, level: w.precipMmh >= pTh * 3 ? 'nogo' : w.precipMmh >= pTh ? 'caution' : 'go', why: w.precipMmh >= pTh ? `${pTh}mm/h 以上` : '' });
  /* 視程は数値予報に無い。ライブカメラで見る前提なので、常に確認をうながす */
  items.push({ key: 'vis', label: '視程', value: 'カメラで確認', level: 'none', why: '数値予報に視程が無いため判定しない' });
  return { level: worst(items.filter(i => i.key !== 'vis').map(i => i.level)), items, elevFt };
}
